...
function repertoryMenu()
    options.menu_subtitle = "Directory"
    ClearMenu()
    Menu.addButton("Add a Number", "newNumero", nil )
    Menu.addButton('Return', 'phoneMenu', nil )
...
function repertoryContact(contact)
    ...
    Menu.addButton('Return', 'repertoryMenu', nil )
...
function msgMenu(msg)
    ...
    Menu.addButton("Read", "readMsg", {msg = msg.msg, name = msg.name, date= msg.date, has_read = msg.has_read, receiver_id = msg.receiver_id})
    Menu.addButton("Respond", "respondTo", msg.owner_id)
    Menu.addButton("Delete Message", "deleteMsg", {msg = msg.msg, name = msg.name, date= msg.date, has_read = msg.has_read, receiver_id = msg.receiver_id, owner_id = msg.owner_id})
    Menu.addButton("Return", "messageryMenu", nil )
end
...
function callTroubleshooters()
    ...
    ClearMenu()
    Menu.addButton("Motorcycles", "callTroubleshootersAction", {type="moto", fn="mecano:callMecano"})
    ...
    Menu.addButton("Cancel Call", "callTroubleshootersAction", {type=nil, fn="mecano:cancelCall"})
    Menu.addButton("Return", "phoneMenu", nil )
end
...
function callTaxis()
    ...
    ClearMenu()
    Menu.addButton("For 1 Person", "callTaxisAction", {type="1 personne", fn="taxi:callService"})
    Menu.addButton("For 2 People", "callTaxisAction", {type="2 personne", fn="taxi:callService"})
    Menu.addButton("For 3+ People", "callTaxisAction", {type="3 personne", fn="taxi:callService"})
    Menu.addButton("Cancel Call", "callTaxisAction", {type=nil, fn="taxi:cancelCall"})
    Menu.addButton("Return", "phoneMenu", nil )
end
...
function cleanMemoryMenu()
    options.menu_subtitle = "Empty Memory"
    ClearMenu()
